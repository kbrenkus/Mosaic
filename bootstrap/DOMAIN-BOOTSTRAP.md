# DOMAIN-BOOTSTRAP-PROTOCOL v0.23
...
- **Tool epistemology in §0:** For domains with 4+ MCP-accessible tools, add a compact block (~1-2 lines per tool) teaching what each tool captures, misses, and means diagnostically in this domain's context. This is Level 4 micro (individual tool nature) complementing the signal hierarchy's Level 4 macro (category reasoning). Keep domain-specific -- the same tool captures different signals in different domains. Place after the signal hierarchy. Size ceiling: ~12 lines.
