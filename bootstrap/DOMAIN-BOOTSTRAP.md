# DOMAIN-BOOTSTRAP-PROTOCOL v0.26
...
- **Worked example in §0:** Each domain QUICK file should include a worked example as an action-oriented recipe with embedded diagnostic labels. Recipe format creates investigation momentum that transfers beyond the modeled query to all queries in the conversation; toolkit briefings create deliberation posture that reduces investigation depth on non-modeled queries (Build 11, A-023). Include concrete search targets AND parenthetical diagnostic labels: "Outlook: vendor emails, scheduling decisions (highest currency)" not just "Outlook (highest currency)." Include a cross-reference prompt ("where do the signals disagree?"). Format: "**Worked example:** *[query]* → retrieve [section] for baseline → [tool]: [concrete targets] ([diagnostic label]) → ... → Cross-reference: where do the signals disagree? **Then talk** — [synthesis frame]."
