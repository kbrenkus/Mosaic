...
- **WHY-annotations in §8A:** Each "Also Check" entry should include a brief reason (1-5 words) explaining what that tool uniquely surfaces. "SharePoint: conference docs" -> "SharePoint: strategy docs and budgets." These help human readers understand the table's rationale and provide marginal context for agents. For investigation behavior change, the signal source hierarchy (Level 4) is more effective than per-tool annotations (see MOSAIC-PRINCIPLES A-023). **Critical: no empty "Also Check" cells.** Empty cells are an active negative signal -- the agent treats them as permission to stay in the primary tool. Every query type row must have at least one "Also Check" entry, even if it is "Teams: team discussions if relevant." Evidence: M-TOOL-4 breadth 1/4 to 3/4 from filling one empty cell (Build 11.5).
- **Worked example in §0:** Each domain QUICK file should include a worked example as an action-oriented recipe with concrete search targets. Recipe format creates investigation momentum that transfers beyond the modeled query to all queries in the conversation; toolkit briefings create deliberation posture that reduces investigation depth on non-modeled queries (Build 11, A-023). Use concrete search targets per tool: "Outlook: vendor emails, scheduling decisions, speaker confirmations" not just "Outlook." Do NOT embed parenthetical diagnostic labels like "(highest currency)" or "(planned state)" -- these introduce micro-deliberation that fragments the action arc and costs 2-3 points (Build 12, A-023). The signal source hierarchy paragraph provides epistemological framing separately. Include a cross-reference prompt ("where do the signals disagree?"). Format: "**Worked example:** *[query]* → retrieve [section] for baseline → [tool]: [concrete targets] → ... → Cross-reference: where do the signals disagree? **Then talk** — [synthesis frame]."
- **Signal source hierarchy in §0:** Categorize the domain's tools by how data is created: real-time signal (M365 -- work as it happens), documented signal (PM tools -- work as recorded), creative production (design tools -- deliverable state), external (web -- public context), curated (reference files -- validated knowledge). Teach the agent WHY to prefer real-time for "what's happening" queries and to diagnose cross-category gaps. This is curriculum (reasoning context), not a directive (tool order). The gap between real-time and documented signal is itself diagnostic -- it reveals where documentation hasn't caught up to reality. Include an absence-query pivot sentence: "When repeated searches return empty results, the absence itself is the signal -- pivot to adjacent entities or upstream capabilities rather than refining search terms." This prevents the refinement loop where agents keep rewriting search terms for queries about things that genuinely don't exist (Build 12 -- broke a 4-round ceiling on absence queries). See MOSAIC-PRINCIPLES A-023.
- **Tool epistemology caution:** Ontological catalogs (listing what each tool captures/misses) are Level 4b content -- they catalog properties but do NOT change agent behavior (Build 9, A-023). The signal hierarchy (Level 4a reasoning framework) is the effective mechanism. Do NOT add per-tool description blocks to SS0 expecting behavioral change. If domain stewards want tool descriptions for human readability, place them in the full reference file (not QUICK) where they don't compete for cognitive foreground.
- **Investigation behavior note:** Behavioral directives ("[question type] -> [tool]") fail for tool selection behavior (MOSAIC-PRINCIPLES A-023, Level 1). **Exception:** Targeted directives addressing specific behavioral patterns with specific alternative actions can work -- e.g., "when repeated searches return empty, pivot to adjacent entities" broke a 4-round refinement loop (Build 12). Broad directives ("search more tools") still fail. Signal source hierarchy (Level 4 macro) teaches category reasoning. Tool epistemology (Level 4 micro) teaches individual tool nature. Worked examples (Level 3) control which tools the agent considers relevant but do NOT control ordering -- the agent determines sequence from structural judgment. Do not prescribe ordering in examples or directives.
...
7. **Optional: solicit agent phenomenological feedback.** Ask: "Which files feel navigated (consulted) vs. absorbed (internalized)?" Content reported as navigated is a pruning candidate (see MOSAIC-PRINCIPLES A-019, A-020, A-021). For deeper mechanism insight, ask: "Can you describe the phenomenology of what you experience when running these queries?" Agent self-reports correlate with empirical scores and reveal causal mechanisms not visible from scores alone -- e.g., tool preference ("HubSpot feels crisp, Outlook muddy"), absence friction ("proving absence is epistemically uncomfortable"), recipe absorption ("following its arc naturally"). Compare phenomenological observations to scored dimensions; design interventions targeting reported friction points (Build 11, A-023).
