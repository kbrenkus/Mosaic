# DOMAIN-BOOTSTRAP-PROTOCOL v0.24
...
- **Worked example in §0:** Each domain QUICK file should include a 2-3 line worked example as a toolkit briefing: trigger query, retrieval baseline, which tools matter and what each contributes, synthesis with gap diagnosis. Design as toolkit briefings (tools + diagnostic value), not recipes (steps in order) -- agents absorb tool coverage from examples but determine sequence from their own judgment (MOSAIC-PRINCIPLES A-023). Format: "**Worked example:** *[query]* → retrieve [section] for baseline. Investigate: [tool] ([what it reveals]), [tool] ([what it reveals]). Cross-reference and synthesize."
...
- [ ] QUICK §0 includes worked example as toolkit briefing (trigger -> retrieval -> tools with diagnostic value -> synthesis with gap diagnosis; no prescribed ordering)
- [ ] QUICK §0 includes tool epistemology block if domain has 4+ MCP tools (what each captures, misses, diagnostic value)
