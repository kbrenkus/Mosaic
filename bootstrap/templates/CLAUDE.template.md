...
- **Phase 6.5 (Curated Zone Baseline):** After first pipeline cycle, classify outputs by zone per MOSAIC-OPERATIONS §4.8. Assess zone weighting (curated-heavy vs interpretive-heavy). Establish write-back baseline.
- **Phase 7 (Validation):** Pre-build baseline and post-build comparison recorded in testing/. Use test plan scoring rubric format.
- **Phase 7.8 (Learning Loop Specification):** Define delta categories, cycle comparison patterns, steward calibration triggers, and write-back tracking per MOSAIC-OPERATIONS §6.7-§6.8.
...
- **`[FRAMEWORK]` delta awareness.** When a domain expert or steward shares interpretive knowledge during a conversation — evaluation criteria, governance rules, contextual reasoning, anti-patterns — emit a `[FRAMEWORK]` delta targeting the interpretive zone. Examples: "We evaluate BaaS deals differently than consulting" → `[FRAMEWORK]` delta capturing the evaluation distinction. "That vendor is on probation after the security incident" → `[FRAMEWORK]` delta capturing governance context. These capture expert knowledge that would otherwise evaporate when the conversation ends. Route to Intelligence Queue. Do NOT auto-integrate — interpretive zone changes require steward validation. See MOSAIC-OPERATIONS §2.2 for delta specification and §6.8 for steward calibration protocol.
