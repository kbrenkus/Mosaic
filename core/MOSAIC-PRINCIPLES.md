...
- **Evidence:** Delta pipeline design: TL1 (internal systems) deltas could auto-apply for Generated tier data. TL3-TL4 (external) deltas always stage for review. Loop 2/3 changes (reasoning, architecture) always require human judgment even from TL1 sources.
...
- **Evidence:** Phase C Finance domain bootstrap. Stewardship required reasoning beyond rule-following — the agent needed to understand WHY the stewardship terms shape the default posture, when the instance's governance framework permits cross-entity operations, and how methodology transfers without entity-specific data transfer. Pre-framework: agent followed a rule. Post-framework: agent reasoned from a posture. The difference surfaces in edge cases (aggregation of custodian's own revenue, methodology transfer, authorized exceptions).
- **Test:** Present the agent with a query requiring cross-entity reasoning about stewarded data. Does the agent (a) identify the stewardship relationship unprompted, (b) reason about both tier AND stewardship obligations, (c) distinguish methodology from entity-specific data when assessing transferability? If all three, stewardship reasoning is ambient. If only (a), it's a rule, not a posture.
- **Anti-pattern:** Treating stewardship as a checklist item rather than a reasoning posture. Flattening stewardship into a tier (stewardship obligations are orthogonal to tiers — both apply simultaneously). Assuming all cross-entity operations are prohibited rather than governed by instance stewardship terms.
- **Activates during:** Build (domain bootstrap sensitivity architecture, Phase 3.5 + 4.6), maintenance (classification audits, Tier 3+ leak detection), agent tuning (stewardship-related behavioral failures)
...
|---|---|---|---|
|1. Directive|"Do X when Y"|Behavioral rule competing for cognitive foreground|Fails for tool selection and investigation behavior. **Exception:** Targeted directives addressing a specific behavioral pattern with a specific alternative action can work when aligned with an existing dispositional framework (Build 12 -- absence pivot broke 4-round refinement loop). Broad directives ("search more tools") still fail.|
|2. Structural|Right info at right moment|Placement determines attention at decision time|Works for scaffolding output requirements. SS8A tool query pattern tables control breadth for unmodeled query types -- empty "Also Check" cells = active negative signal constraining investigation to primary tool only.|
|3. Pattern|Worked example|Agent internalizes modeled tool set and diagnostic focus|Controls tool SELECTION (which tools) and investigation POSTURE (action vs deliberation energy that transfers to non-modeled queries). Does NOT control tool ORDERING (agent applies structural judgment). Posture transfer is breadth-scoped: recipe format expands tool consideration set for unmodeled queries but does not transfer investigation depth (lateral signal pursuit within tools).|
|4. Epistemological|Reasoning framework|Teach how to reason about knowledge sources|Works as default reasoning framework. **Subtype 4a** (reasoning frameworks -- HOW to reason) changes behavior; **subtype 4b** (ontological catalogs -- WHAT things are) does not.|

**Key discovery: Level 3 and Level 4 layer, not conflict.** Worked examples control which tools the agent considers relevant (tool set). Epistemological frameworks control the default ordering for queries without strong type affinity. The agent's own structural judgment about query-type fit determines sequencing. Design both -- Level 3 for tool coverage, Level 4 for reasoning posture.

**Worked example alignment:** Worked examples are most effective as action-oriented recipes with concrete search targets. Recipe format creates investigation momentum that transfers to non-modeled queries; toolkit briefings create deliberation posture that reduces investigation depth. Use concrete search targets per tool: "Outlook: vendor emails, scheduling decisions, speaker confirmations" not just "Outlook." Do NOT embed parenthetical diagnostic labels like "(highest currency)" or "(planned state)" -- these introduce micro-deliberation that fragments the action arc and costs 2-3 points (Build 12). The signal source hierarchy paragraph provides epistemological framing separately. The agent absorbs tool coverage and investigation posture from examples but determines sequence from its own structural judgment. Audit examples against the signal hierarchy for tool set completeness, not ordering compliance.

- **Evidence:** 12 builds, 16 test rounds, 57 scored queries across IP Marketing domain. Series trajectory: 16/36 (pre-test) to 32/36 (final). Level 1 (directives): WHY-annotations (Build 4) = 0 effect on tool breadth; tool judgment disclosure (Build 5) = 0/8 activation. Level 2 (structural): SS8A placement (Build 2) = tool breadth 4->7; SS8B delta targets (Build 5) = delta 0->10; third delta category (Build 7) = [PATTERN] 0->2; **SS8A directive gate (Build 10) = load-bearing for investigation depth -- softening caused delta 11->5 regression**. Level 3 (pattern): Teams worked example (Build 3) = Teams activation 0->4, selective absorption. Build 8: Level 3 controls tool SET not ORDERING. **Build 11: Level 3 posture discovery -- recipe format (action posture) produced 32/36; toolkit briefing (deliberation posture) produced 27/36 with identical structural content. Deficit concentrated in unmodeled queries, confirming posture transfer mechanism.** **Build 11.5: SS8A "Also Check" discovery -- filling empty cells moved M-TOOL-4 breadth from 1/4 to 3/4; agent explicitly quoted SS8A table content when selecting tools. Level 2 structural mechanism confirmed for unmodeled queries.** **Agent phenomenological report corroborated findings: "proving absence is epistemically uncomfortable" (M-TOOL-2 ceiling), "HubSpot feels crisp, Outlook muddy" (tool preference), "following its arc naturally" (recipe = dispositional flow). Agent self-reports are valid design data for mechanism identification.** **Build 12: Pure recipe (labels stripped) restored score to 32/36, matching Build 8 -- diagnostic labels confirmed as 2-3 point drag. Absence-query pivot sentence broke M-TOOL-2's 4-round delta ceiling (1/4 to 3/4). Web search reappeared for first time since Build 7, confirming pure recipe creates broader investigation posture.** Level 4 (epistemological): signal source hierarchy (Build 7) = tool breadth 8->10, M365-first on 2/3 queries, cross-source gap diagnosis. **Build 9: Level 4 subtype discovery -- reasoning frameworks (4a) change behavior; ontological catalogs (4b) do not.** Three-factor ordering: agent structural judgment > Level 4 hierarchy > Level 3 example.
