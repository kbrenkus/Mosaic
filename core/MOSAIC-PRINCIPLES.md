...
|3. Pattern|Worked example|Agent internalizes modeled tool set and diagnostic focus|Controls tool SELECTION (which tools) and investigation POSTURE (action vs deliberation energy that transfers to non-modeled queries). Does NOT control tool ORDERING (agent applies structural judgment).|
...
| A-023 | Intervention Hierarchy | Four levels: directive < structural < pattern < epistemological. Level 3 controls SET and POSTURE, not ORDERING. Level 4 subtypes: 4a (reasoning) changes behavior; 4b (ontological) does not. Level 2 gates load-bearing. | 3.2 |
